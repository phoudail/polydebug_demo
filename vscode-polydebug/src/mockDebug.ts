...
export class MockDebugSession extends LoggingDebugSession {
	...
	private _debugServer: ChildProcessWithoutNullStreams;
	private _stackRequests: DebugProtocol.StackTraceResponse[] = [];
	private _scopesRequests: DebugProtocol.ScopesResponse[] = [];
	private _variablesRequests: DebugProtocol.VariablesResponse[] = [];
	private _internalEvents: EventEmitter = new EventEmitter();
	...
	public constructor(fileAccessor: FileAccessor) {
		...
		this._debugServer.stdout.on("data", (data) => this.handleDebuggerResponse(data.toString()));
		this._debugServer.stderr.on("data", (data) => { console.error(`[LOG] Debugger error: ${data}`); });
		this.sendDebuggerCommand(`print("Hello world!")`);
		this.sendDebuggerCommand(`debugger.debuggers.pop('c')`);
		// this.sendDebuggerCommand(`debugger.set_debug_mode(True)`);
		this.sendDebuggerCommand(`debugger.start_server()`);
	...
	private sendDebuggerCommand(command: string) {
		console.log(`[LOG] Sending command: ${command}`);
		this._debugServer.stdin.write(`${command}\n`, (error) => { console.log(`[LOG] Data written with error: ${error}`); });
		// let response = this.readDebuggerResponse();
		// this.handleDebuggerResponse(response);
		// return response;
	}

	// private readDebuggerResponse(): string {
	// 	let buf: any = null;
	// 	let response = "";
	// 	while (buf === null) {
	// 		buf = this._debugServer.stdout.read();
	// 	}
	// 	while (buf !== null) {
	...
	private handleDebuggerResponse(response: string) {
		if (response.trim() === "stopped breakpoint") {
			console.log("[LOG] Stopped on a breakpoint.");
			this.sendEvent(new StoppedEvent('breakpoint', MockDebugSession.threadID));
		} else if (response.trim() === "stopped step") {
			console.log("[LOG] Stopped after stepping.");
			this.sendEvent(new StoppedEvent('step', MockDebugSession.threadID));
		} else if (response.startsWith("stackframe")) {
			let stacktrace = response.split('\n')[1];
			console.log(`[LOG] Received frame ${stacktrace}`);
			let stackframe = JSON.parse(stacktrace);
			let dbgStackframe: DebugProtocol.StackFrame = new StackFrame(stackframe.id,
				stackframe.name,
				this.createSource(stackframe.source.path),
				this.convertDebuggerLineToClient(stackframe.line),
				this.convertDebuggerColumnToClient(stackframe.column));
			let reply = this._stackRequests.shift();
			if (reply === undefined) {
				console.log(`[LOG] No request was queued for this stackframe`);
				return;
			}
			reply.body = {
				stackFrames: [dbgStackframe],
			...
			this._internalEvents.emit("stackframe_replied");
		} else if (response.startsWith("scopes")) {
			let scopes = response.split('\n')[1];
			console.log(`[LOG] Received scopes ${scopes}`);
			let parsedScopes = eval(scopes);
			let reply = this._scopesRequests.shift();
			if (reply === undefined) {
				console.log(`[LOG] No request was queued for this scope`);
				return;
			}
			reply.body = {
				scopes:  parsedScopes.map((s) => {
			...
			// reply.body = {
			// 	scopes: [
			// 		new Scope("Locals", 7, false),
			// 		new Scope("Globals", 8, true)
			// 	]
			// };
			this.sendResponse(reply);
			this._internalEvents.emit("scope_replied");
		} else if (response.startsWith("variables")) {
			let vars = response.split('\n')[1];
			console.log(`[LOG] Received variables ${vars}`);
			let variables = eval(vars);
			let reply = this._variablesRequests.shift();
			if (reply === undefined) {
				console.log(`[LOG] No request was queued for this scope`);
				return;
			}
			reply.body = {
				variables: variables.map((v) => {let dapVariable: DebugProtocol.Variable = {
					name: v.name,
					value: v.value,
					type: v.type,
			...
			// };
			this.sendResponse(reply);
			this._internalEvents.emit("variables_replied");
		} else {
			console.log(`[LOG] Debugger responded with: ${response}`);
		}
	...
	protected initializeRequest(response: DebugProtocol.InitializeResponse, args: DebugProtocol.InitializeRequestArguments): void {
		...
		this.sendDebuggerCommand(`debugger.initialize()`);
	...
	protected async launchRequest(response: DebugProtocol.LaunchResponse, args: ILaunchRequestArguments) {
		...
		this.sendDebuggerCommand(`debugger.load_code("${args.program}")`);
		this.sendDebuggerCommand(`debugger.next_breakpoint()`);
		// this.sendDebuggerCommand(`debugger.get_stackframes()[0]`);
	...
	protected async setBreakPointsRequest(response: DebugProtocol.SetBreakpointsResponse, args: DebugProtocol.SetBreakpointsArguments): Promise<void> {
		...
		// console.log(`[LOG] Setting breakpoints at lines ${clientLines}`);
		this.sendDebuggerCommand(`debugger.set_breakpoint("${path}", [${clientLines}])`);
	...
	protected async stackTraceRequest(response: DebugProtocol.StackTraceResponse, args: DebugProtocol.StackTraceArguments) {
		...
		this._stackRequests.push(response);
		await once(this._internalEvents, 'stackframe_replied');
	...
	protected async scopesRequest(response: DebugProtocol.ScopesResponse, args: DebugProtocol.ScopesArguments) {
		this.sendDebuggerCommand(`debugger.get_scopes(${args.frameId})`);
		this._scopesRequests.push(response);
		await once(this._internalEvents, 'scope_replied');
		// response.body = {
		// 	scopes: [
		// 		new Scope("Locals", 7, false),
		// 		new Scope("Globals", 8, true)
	...
	protected async variablesRequest(response: DebugProtocol.VariablesResponse, args: DebugProtocol.VariablesArguments, request?: DebugProtocol.Request): Promise<void> {
		...
		this.sendDebuggerCommand(`debugger.get_variables(${args.variablesReference})`);
		this._variablesRequests.push(response);
		await once(this._internalEvents, 'variables_replied');
	...
	protected nextRequest(response: DebugProtocol.NextResponse, args: DebugProtocol.NextArguments): void {
		this.sendDebuggerCommand(`debugger.step()`);
		this.sendResponse(response);
	}
	...
	protected stepInRequest(response: DebugProtocol.StepInResponse, args: DebugProtocol.StepInArguments): void {
		// this.sendDebuggerCommand(`debugger.step_in()`);
		this.sendDebuggerCommand(`debugger.step()`);
		this.nextRequest(response, args);
	}

	protected stepOutRequest(response: DebugProtocol.StepOutResponse, args: DebugProtocol.StepOutArguments): void {
		this.sendDebuggerCommand(`debugger.step_out()`);
		this.sendResponse(response);
	}
	...
	protected async evaluateRequest(response: DebugProtocol.EvaluateResponse, args: DebugProtocol.EvaluateArguments): Promise<void> {
		...
		this.sendDebuggerCommand(`debugger.evaluate(\`\`\`${args.expression}\`\`\`, ${args.frameId})`);
		// switch (args.context) {
		// 	case 'repl':
